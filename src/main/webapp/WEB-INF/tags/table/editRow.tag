...
<button class="btn btn-sm btn-primary btn-inquire" data-toggle="tooltip" data-placement="left" onclick="edit()" title="修改"><i class="fa fa-file-text-o"></i> ${label==null?'修改':label}</button>
                        </button>
...
$(document).ready(function() {
	...
	function edit(){
		  ...
		  if(size == 0 ){
				top.layer.alert('请至少选择一条数据！', {icon: 0, title:'提示'});
				return;
			  }

		  if(size > 1 ){
				top.layer.alert('只能选择一条数据！', {icon: 0, title:'提示'});
				return;
			  }
		    var id =  $("#${id} tbody tr td input.i-checks:checkbox:checked").attr("id");
		    openDialog("修改"+'${title}',"${url}?id="+id,"${width==null?'800px':width}", "${height==null?'595px':height}","${target}");
		}
</script>